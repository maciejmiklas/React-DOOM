...
class EditWadTag extends Component {
	...
	render() {
		const {wad, validateWadName, handleNameChange, viewWad} = this.props;
		return (
			<Navigation>
				<Row>
				...
				<Row>
					<Col sm={{span: 6, offset: 3}}>
						<Card bg="card">
							<Card.Body>
								<Button className="input-button" block onClick={viewWad}>
									WAD Viewer
								</Button>
							</Card.Body>
						</Card>
...
const dispatchToProps = (dispatch, wadName) => {
	...
	return {
		...
		viewWad: () => {
			dispatch(actionGotoPage(PAGES.WAD_VIEW, {wadName:wadNameStr}));
		},
	};
};
